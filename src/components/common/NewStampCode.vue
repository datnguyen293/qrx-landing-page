...
<template>
  <div class="qrx-container m-auto overflow-hidden flex flex-col justify-between">
    ...
    <div class="m-auto text-center mt-6 px-4">
      <div class="font-bold text-3xl mb-2 qrx-text--warning qrx-line-break">{{ !isEmpty(titleError) ? titleError : $t('common.stamp_new_title')}}</div>
      <div class="text-[16px] qrx-text--warning qrx-line-break">{{ !isEmpty(contentError) ? contentError : $t('common.stamp_new_content') }}</div>
    </div>
  </div>
