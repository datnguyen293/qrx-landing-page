...
<template>
  ...
  <template v-else-if="status === STATUS_VERIFY.OVER_LIMITED">
    <div class="px-3 py-5 qrx-bg--warning">
      <div class="logo-success flex justify-center mb-3">
        <img v-if="!isEmpty(logoUrl)" :src="logoUrl" alt="Logo stamp over limited" class="!max-w-[100px] !max-h-[81px]"/>
        <template v-else>
          <svg width="100"
               height="81" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
      ...
      <div class="text-center text-[#FCFCFC]">{{ messageStampCode?.content || $t('common.limit_scan_content') }}</div>
    </div>
  </template>
